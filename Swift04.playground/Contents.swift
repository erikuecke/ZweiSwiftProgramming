//: Playground - noun: a place where people can play

import Cocoa

print("The maximum Int value is \(Int.max)")
print("The minimum Int value is \(Int.min)")
print("The maximum value for a 32-bit integer is \(Int32.max)")
print("The minimum value for a 32-bit ingteger is \(Int32.min)")

// Maximum and minimum values for unsigned integers
print("The maximum UInt value is \(UInt.max)")
print("The minimum UInt value is \(UInt.min)")
print("The maximum value for a 32-bit unsigned integer is \(UInt32.max)")
print("The minimum value for a 32-bit unsigned integer is \(UInt32.min)")

// CREATING INTEGER INSTANCES
// Declaring Int explicitly and implicitly

let numberOfPages: Int = 10 // Declares the type explicitly
let numberOfChapters = 3 // Also of type int, but inferred by the compiler

// Declaring other integer types explcitly
let numberOfPeople: UInt = 40
let volumeAdjustment: Int32 = -1000

// Delcaring integer types with invalid values
// Trouble ahead!
//let firstBadValue: UInt = -1
//let secondBadValue: Int8 = 200

// OPERATIONS ON INTEGERS
// Performing basic operations
print(10 + 20)
print(30 - 5)
print(5 * 6)

// Order of operations

print(10 + 2 * 5)
print(30 - 5 - 5 )
print((10 + 2) * 5)
print(30 - (5 - 5))

// INTEGER DIVISION
// Integer division can give unexpected results
print(11 / 3)

// Reamainders

print(11 % 3)
print(-11 % 3)


// OPERATOR SHORTHAND
// Combining addition or subtraction and assignment
var x = 10
x += 10
print("x has had 10 added to it and is now \(x)")
x -= 5
print("x has had 5 subtracted from it and is now \(x)")

// OVERFLOW OPERATORS
// Solve for z
let y: Int8 = 120
let z = y &+ 10
print("120 &+ 10 is \(z)")

// CONVERTING BETWEEN INTEGER TYPES
// Adding values of different types
let a: Int16 = 200
let b: Int8 = 50
let c = a + Int16(b)

// FLOATING POINT NUBMERS
// Declaring floating point number types

let d1 = 1.1
let d2: Double = 1.1
let f1: Float = 100.3

print(10.0 + 11.4)
print(11.0 / 3.0)

if d1 == d2 {
    print("d1 and d2 are the same!")
}

// Unexpected results
print("d1 + 0.1 is \(d1 + 0.1)")
if d1 + 0.1 == 1.2 {
    print("d1 + 0.1 is equal to 1.2")
}














